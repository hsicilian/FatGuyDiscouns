...
import { updateCustomerProfileByAdminAction } from "../../app/actions/customers/profile";

const initialState: FormActionState = {
  ok: true,
  message: "Update the customer's address and timezone.",
};

export function CustomerProfileAdminForm({
  customerId,
  defaultStreet,
  defaultCity,
  defaultRegion,
  ...
  return (
    <form action={formAction} style={{ display: "grid", gap: 12 }}>
      ...
      <button disabled={isPending} style={{ background: "#bb4d00", color: "#fff", border: 0, borderRadius: 999, padding: "12px 16px", fontWeight: 700 }}>
        {isPending ? "Saving..." : "Update Customer Profile"}
      </button>
